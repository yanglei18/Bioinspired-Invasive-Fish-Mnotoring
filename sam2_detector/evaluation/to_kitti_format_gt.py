...
try:
    ...
    from sam2_detector.utils.annotation_io import (
        get_existing_aggregated_annotation_path,
        load_prediction_sequences,
    )
    from sam2_detector.utils.fish_label_map import (
        FINE_CLASSES,
        LABEL_ID_TO_NAME,
        get_fish_class_name,
        normalize_fish_class_name,
...
def get_frame_range(input_file: str) -> tuple[int, int]:
    """Get the minimum and maximum frame IDs from a label file."""
    min_frame = float("inf")
    max_frame = -1
...
def count_labels(input_file: str, target_class: str | None = None) -> Counter:
    """Count the number of objects per category in a label file."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="work_dirs/sam2_prediction/kitti_eval_run/gt/label_02",
