...
try:
    ...
    from sam2_detector.evaluation.detection import run_detection_coarse, run_detection_fine
    from sam2_detector.evaluation.tracking import build_tracking_frame_filters, run_tracking
    from sam2_detector.evaluation.to_kitti_format_gt import collect_gt_sequence_names
    from sam2_detector.utils.annotation_update import UpdateConfig, run_update
    from sam2_detector.utils.annotation_io import get_existing_aggregated_annotation_path, load_prediction_sequences
    from sam2_detector.utils.fish_label_map import LABEL_ID_TO_NAME
    from sam2_detector.evaluation.reporting import (
        build_combined_text_report,
        to_jsonable,
        write_json_report,
        write_text_report,
...
def validate_json_gt_dir(gt_label_dir: Path) -> dict[str, dict]:
    ...
    raise FileNotFoundError(
        ...
        "Expected perdiction.json, a single sequence JSON, or legacy per-frame JSON directories."
    )
