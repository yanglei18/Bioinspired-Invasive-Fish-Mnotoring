...
def compute_ap_from_precisions(precision_values):
    return compute_ap11_from_precisions(precision_values)
...
def resolve_display_class_title(cls, terminal_label):
    if terminal_label and terminal_label.startswith("fine:"):
        return terminal_label.split(":", 1)[1]
    return cls
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            gt_data[frame].append({
                ...
                'track_id': int(line[1]),
                'class': FISH_CLASS,
                'trunc': float(line[3]),  # 截断程度
                'occ': float(line[4]),    # 遮挡程度
                'alpha': float(line[5]),  # 观测角度
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            pred_data[frame].append({
                ...
                'dimensions': [float(x) for x in line[10:13]],  # [height, width, length]
                'location': [float(x) for x in line[13:16]],    # [x, y, z]
                'score': float(line[17]) if len(line) > 17 and line[17] != '-1' else 1.0  # 检测置信度，如果不存在则默认为1.0
            })
    return pred_data
