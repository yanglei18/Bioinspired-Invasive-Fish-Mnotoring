...
SAM2_DETECTOR_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = SAM2_DETECTOR_ROOT.parent
DEFAULT_CONFIG_PATH = SAM2_DETECTOR_ROOT / "configs" / "default.yaml"
...
class EvaluationConfig:
    pred_json_dir: str = "work_dirs/sam2_prediction/annotation_det"
    gt_label_dir: str = "data/robotfish-data/label_rectified.json"
    inference_json: str | None = None
    work_dir: str = "work_dirs/sam2_prediction/kitti_eval_run"
...
def load_detector_config(config_path: str | Path | None = None) -> Sam2DetectorConfig:
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "PyYAML is required to load sam2_detector YAML configs. "
...
def build_detector_config(raw_config: Mapping[str, Any]) -> Sam2DetectorConfig:
    ...
    outputs = OutputConfig(
        det_dir=str(outputs_section.get("det_dir", "work_dirs/sam2_prediction/annotation_det")),
        mask_dir=str(outputs_section.get("mask_dir", "work_dirs/sam2_prediction/annotation_mask")),
        masked_image_dir=_optional_str(outputs_section.get("masked_image_dir", "work_dirs/sam2_prediction/masked_image")),
        masked_video_dir=_optional_str(outputs_section.get("masked_video_dir", "work_dirs/sam2_prediction/masked_video")),
        instance_image_dir=str(outputs_section.get("instance_image_dir", "work_dirs/sam2_prediction/instance_image")),
        instance_video_dir=str(outputs_section.get("instance_video_dir", "work_dirs/sam2_prediction/instance_video")),
    )
    crop = CropConfig(
    ...
    evaluation = EvaluationConfig(
        ...
        work_dir=str(evaluation_section.get("work_dir", "work_dirs/sam2_prediction/kitti_eval_run")),
