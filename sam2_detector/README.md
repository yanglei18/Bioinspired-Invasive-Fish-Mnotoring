...
This directory contains the fish detection, tracking, export, and evaluation pipeline used in our project. For open-source use, the recommended user-facing entrypoints are:

- `ground_sam2_plus_crop_instance.py`: main inference / tracking / crop export pipeline
...
├── ground_sam2_plus_crop_instance.py   # main inference entrypoint
...
```bash
conda create -n sam2_detector python=3.10
...
```text
sam2_detector/configs/default.yaml
```
...
- `work_dirs/sam2_prediction/annotation_det/perdiction.json`
- `work_dirs/sam2_prediction/instance_image/*.webp`
- `work_dirs/sam2_prediction/instance_video/*.mp4`
...
- `work_dirs/sam2_prediction/instance_image/PRO_VID_20250409_135930_00_014_012_3_006600.webp`
- `work_dirs/sam2_prediction/instance_video/PRO_VID_20250409_135930_00_014_012_3.mp4`

## Prediction Update During Evaluation

If you classify exported instance videos externally and want to evaluate the updated fine-grained labels, set `evaluation.inference_json` in `sam2_detector/configs/default.yaml` or pass `--inference_json` for a one-off run:
...
- `evaluation.pred_json_dir`: prediction directory containing `perdiction.json`, default `work_dirs/sam2_prediction/annotation_det`
