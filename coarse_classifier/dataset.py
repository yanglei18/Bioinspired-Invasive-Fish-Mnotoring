...
class FishDataset(Dataset):
    ...
    def get_data_loaders(train_dir, val_dir=None, batch_size=32, image_size=224, 
                         num_workers=4, return_paths=False, use_grayscale=False):
        """
        ...
        if val_dir and os.path.exists(val_dir):
            ...
            # Use BalancedBatchSampler for validation as well
            val_sampler = BalancedBatchSampler(val_dataset, batch_size)
            
            val_loader = DataLoader(
                val_dataset,
                batch_sampler=val_sampler,  # Use batch_sampler instead of batch_size and shuffle
                num_workers=num_workers,
                pin_memory=True
            )
...
class BalancedBatchSampler(Sampler):
    """
    Ensures each batch contains samples from all classes with balanced distribution
    """
    def __init__(self, dataset, batch_size: int):
        ...
        for idx in range(len(dataset)):
            if isinstance(dataset, ClassPairFishDataset):
                _, _, class_idx = dataset.samples[idx]
            else:  # FishDataset
                _, class_idx, _ = dataset.samples[idx]
        ...
        if self.samples_per_class == 0:
            ...
            print(f"Warning: batch_size ({batch_size}) is smaller than num_classes ({self.num_classes}). "
                  f"Setting samples_per_class to 1.")
        
        # Calculate the total number of batches
        min_class_size = min(len(indices) for indices in self.class_indices.values())
        self.total_size = (min_class_size // self.samples_per_class) * self.num_classes * self.samples_per_class
        
    def __iter__(self):
        # Create a copy of class indices to avoid modifying the original
        remaining_indices = {
            class_idx: indices.copy()
            for class_idx, indices in self.class_indices.items()
        }
        
        # Shuffle indices for each class
        for indices in remaining_indices.values():
            random.shuffle(indices)
        
        batches = []
        while True:
            batch = []
            # Try to add samples_per_class samples from each class
            for class_idx in self.class_indices.keys():
                class_indices = remaining_indices[class_idx]
                # Take up to samples_per_class samples
                samples_to_take = min(self.samples_per_class, len(class_indices))
                if samples_to_take == 0:
                    # If any class runs out of samples, start a new epoch
                    return iter(batches)
                
                batch.extend(class_indices[:samples_to_take])
                # Remove used indices
                remaining_indices[class_idx] = class_indices[samples_to_take:]
            
            if batch:
                # Shuffle the batch to avoid having all samples from the same class consecutive
                random.shuffle(batch)
                batches.extend(batch)
            
            # If any class has run out of samples, start a new epoch
            if any(len(indices) < self.samples_per_class for indices in remaining_indices.values()):
                break
